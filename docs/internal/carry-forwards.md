...
| `CF-v2.5-F` (security/QA) | Upstream acknowledgement-window watch (60-day) | **ORPHANED — OVERDUE** | **Stated escalation date 2026-07-08; 51 days past due at this cycle's base (computed, not estimated).** The escalation it specifies has never been performed. This is the single most actionable row in the register |
...
| `CF-v2.5-ARCH-D` | F3 PR outcome evaluation — the v3.0 gate review reads the upstream PR's acknowledgement outcome | v2.5 Phase 1 | `docs/architecture.md` v2.5 carry-forwards section | OPEN | **v3.0 gate work.** Paired with `CF-v2.5-F` (security/QA), which is 51 days overdue — the same upstream PR |
