...
| OT-8 | Confirm `tests/offline-smoke-test.md`'s scorecard is current BEFORE running Scope A (`v2.19.6` "Publish What Shipped" — backfilling `v2.19.4`, `v2.19.5`, then publishing `v2.19.6` itself, three tags in one sitting) | v2.19.6 | Scope A's three `scripts/publish-release.sh` publishes | OPEN — `CONTRIBUTING.md`'s pre-release checklist binds every tag pushed to a current smoke-test scorecard, and this is a maintainer step no CI gate can enforce (`[P1-CORRECTION-5]`, `docs/spec.md`). Scope A pushes three tags in one sitting, not one — the obligation fires three times, not once for the batch. A tag pushed without a current scorecard means the README "15 minutes" hero claim ships unverified for that release. | 0 releases | 0 |
