...
### 0. Four orchestrator-authored errors, all caught by briefed agents — recorded, not smoothed over
...
None of the four reached `main`. All four were caught by the agent the orchestrator briefed, not by the orchestrator itself — which is the discipline this project keeps asking of every reviewed artifact, working when pointed at the orchestrator's own output.
...
### `@qa`-independent finding, corrected after reproduction: the Release Surface gate cannot see the tags it checks for — a sixth instrument-blindness instance this cycle, not a timing race

**Verified this session, not inherited — and revised once, on better evidence, before this retro's own PR opened.** The environment briefing for this retro stated "Release-surface standing gate: 12 checked, 0 failed." That was wrong. My first pass caught the failure but under-diagnosed it as a timing race; the coordinator reproduced CI's actual checkout conditions and found the real cause, which supersedes the race-only reading below. Recorded here as corrected, not silently replaced, per this project's own discipline of showing a fix pass rather than smoothing over the first draft.

**What is true, and stays true, about attempt 1 only:** `gh run list --workflow=release-surface.yml` shows the merge-commit `push`-triggered run (`32419773765`, created `2026-08-20T21:30:48Z`, `conclusion: failure`) fired **52 seconds before** the GitHub Release was published (`2026-08-20T21:31:40Z`) — the job's tag/release check found v2.19.10 genuinely absent at the instant it ran. That is a real, if narrow, explanation, and it fully accounts for attempt 1.

**It does not account for attempts 2 and 3, which is what falsifies the race as the whole story:** a re-run at `2026-08-20T21:52:39Z` — **21 minutes after publish** — also failed, and a fresh `workflow_dispatch` run (`32422621025`, ~`22:15Z`) also failed, both long after the release genuinely existed. CI logs are unretrievable for this repo (`BlobNotFound` on the API, consistently), so the real cause was found by reproducing CI's conditions locally rather than inferring one from timestamps:

```
git clone --depth 1 --no-tags <repo>   # what actions/checkout does by default
git tag | wc -l                        →  0
GH_TOKEN=… bash scripts/verify-release-surface.sh --floor 2.18.0
```

**Every version at or above the floor reports `MISSING-TAG` under this clone — including `v2.19.8` and `v2.19.9`, both demonstrably tagged and published.** The real cause: `.github/workflows/release-surface.yml` uses `actions/checkout@v4.2.2` with **no `fetch-tags: true` and no `fetch-depth: 0`**, so the CI checkout carries zero tags regardless of what exists on `origin` — a pre-existing, standing-gate configuration defect, not caused by this cycle's product changes, and not explained by a merge-timing race.

**One anomaly is recorded as open, not resolved:** the scheduled run against `fd00dd2` at `2026-08-20T07:05:32Z` succeeded, under what should be the identical checkout configuration. No mechanism is asserted for that success here — it contradicts a simple "this gate can never pass in CI" reading, and is left for whoever fixes the checkout config to explain. Also worth preserving for that fix: the script's own error message is internally contradictory — `"no refs/tags/v2.19.10 on origin"` immediately followed by a parenthetical `"(origin has it; local checkout does not — tagged but never pushed?)"`, asserting both that origin lacks the tag and that origin has it, in the same message.

**This is a sixth instance of the same shape this cycle already caught six times over (S1, S2, S13, S14, S15, A4 — an instrument, gate, or fixture that cannot see, or cannot go GREEN on, the actual condition it exists to verify).** Filed under the existing BINDING `docs/patterns.md:32` "Verifier-that-cannot-PASS / unearned-RED" row as a post-promotion occurrence, not a new row — the shape is exactly that row's own already-stated discipline ("prove it can PASS on the actual correct/intended final state… across the realistic input space the gate will actually see in production"), applied to a standing gate that had never been proven against its real CI checkout conditions. **Carry-forward, Tier B, its own cycle — not folded into anything:** fix `release-surface.yml` to `fetch-tags: true` (or `fetch-depth: 0`).
...
- **`release-surface.yml` is checked out with no `fetch-tags: true` and no `fetch-depth: 0`, so CI's checkout carries 0 tags and every version reports `MISSING-TAG` regardless of truth** — reproduced directly (see the dedicated finding above), not a timing race except for the very first post-merge run. **Carry-forward: fix the checkout step to fetch tags — a `.github/workflows/` change, Tier B, its own cycle, not folded into anything.** The `fd00dd2` scheduled run's success under what should be the same configuration is recorded as an open, unexplained anomaly, not a reason to assume the gate is fine. Filed as a post-promotion occurrence under `docs/patterns.md:32` (Verifier-that-cannot-PASS, BINDING), not a new row.
- **`docs/patterns.md` updates landed this cycle:** `:55` "Claim scope wider than its verifying instrument" incremented WATCH 1/3 → 2/3 (this cycle's own ruling, settled at Phase 0.D R2/R3, `@qa` records the increment and does not re-adjudicate it); `:56` "Ambiguous-unit numeric claim" — a genuine third live instance (the `bundle` count: `@architect` 4 → `@pm` 3 → measured 2 user-facing, `pipeline.md` "0.D Deliberation (R3)" row) recorded in the row's Cycles text but **not** incremented past WATCH 2/3, because unlike the two counted instances this one was caught and corrected inside the same phase (Phase 0) it was raised in, the cycle's own contemporaneous record explicitly states it was "not counted as a ship instance," and nothing stale shipped — see the ruling note inline in the row; new WATCH 1/3 row "The cycle's own mandated corrective action was the next defect vector"; new WATCH 1/3 row "Line-pinned prose anchors break silently on insertion"; `:32` "Verifier-that-cannot-PASS / unearned-RED" (already BINDING) given a new v2.19.10 post-promotion occurrence (`release-surface.yml`'s no-tags checkout) — no counter change, BINDING rows don't need one.
