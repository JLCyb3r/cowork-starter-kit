...
| `CF-v2.19.18-PHASE6-DEFERRED` | **Added Phase 6 (@security audit, `release/v2.19.18-collision-safety` @ `4a74afa`).** The Phase 6 audit's BLOCK (1 CRITICAL, 9 WARNING, 7 INFO) resolved as S1 (CRITICAL, firing-control mismatch) and S2/S9 (WARNING, over-claimed promise scope / stale control citations) — all three fixed in this rework, recorded at their own site (§H and the disposition trail in `tests/collision-safety-firing-controls.md`). **The audit's remaining WARNING and INFO items (referenced in the rework instruction as "S3–S8, S10, and all INFO items") were explicitly ruled by @security as genuine but not release-holding, and are deferred to a follow-up cycle alongside `CF-v2.19.18-QUALSILENCE`, `-LEDGERDUR`, and `-STALEECHO`.** @dev recorded this pointer without inventing their technical content: the rework instruction that authorized this pass named their disposition (deferred, non-blocking) but did not include their individual descriptions, and @dev does not have independent access to @security's full Phase 6 audit text — only the full record of S1/S2/S9. **Do not treat this row as a substitute for reading the actual Phase 6 audit record** (the pipeline's own Phase 6 log entry, or a follow-up `@security` pass) before scoping the next cycle; it is a pointer that these items exist and were seen, not a technical summary of what they say. **[UPDATE — Phase 7 (post-merge CI fix), CI red on PR #128's word-count check]:** fixing `templates/workspace-claude-md-template.md`'s word count (456 → 400-ceiling breach) required rewriting its two `_setup-kit/`-dependent lines (Skill swap, Re-run setup) from location-*asserting* to location-*checking* wording — true whether the kit is unarchived (Mode A) or was never in the workspace (Mode B), rather than asserting a fixed path that's false in Mode B. **This closes S10 as a side effect, verified against the `mode-b` fixture (confirmed: no `skills/`, `vendored/`, or `WIZARD.md` at that fixture's root) before and after the change** — not silently absorbed. **S10 is CLOSED.** S3–S9 (excluding the already-closed S1/S2/S9), S11–S17, and all INFO items remain open and deferred; their content is still not reproduced here — see the Council-side record this row already points at | INFO |
