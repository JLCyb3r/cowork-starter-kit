...
|------|-------------|------------|--------------|------|-----------|----------------------------------------------------------------|
| self-apply | One of three required safety skills. It records every change you approve, in order, using a consistent format so entries stay easy to track, and requires you to propose a change, check it, and be able to reverse it before it takes effect. This file can never be changed or moved by this or any other skill — it's on a fixed, protected list that both processes always skip. Starting in v2.19, files whose names start with `self-` are protected the same way, along with the `context/.kit-migrations/` folder. | builtin | 2026-07-22 | 1 | mandatory-infrastructure | 0c77ab20779c79288eb35f3e1059955b566b3460456034b85dc87959a955e9f4 |
| self-archive | One of three required safety skills. It checks whether a file is safe to move before suggesting you archive something old or replaced, confirms the destination makes sense, and never deletes anything — it only moves a file into your archive folder, so nothing is lost and you can always move it back yourself. This file can never be changed or moved by this or any other skill — it's on a fixed, protected list that both processes always skip. Starting in v2.19, `self-upgrade` and the `context/.kit-migrations/` folder are covered by the same protected list. | builtin | 2026-07-22 | 1 | mandatory-infrastructure | 7b12d467dbc1fc73cdd058b0d2bacb06fff7b8f9b07614fa2e478a30edf6d7f9 |
| self-upgrade | One of three required safety skills. It handles moving your setup forward when a newer release of this kit becomes available, and checks the new version against the version you already trust before it takes over — never the other way around. It isn't actively used yet in v2.19, since there's no newer release to move to, but it's installed and ready. This file can never be changed or moved by this or any other skill — it's on a fixed, protected list that both processes always skip. | builtin | 2026-07-22 | 1 | mandatory-infrastructure | a4abd71af9dafe8a267af758c86593a1260426a5387f2825f75512559fdf3e11 |
...
| name | description | source_url | vetting_date | tier | goal_tags | sha256 |
|------|-------------|------------|--------------|------|-----------|----------------------------------------------------------------|
| pull-updates | Checks every skill you've installed against the copies included with this kit on your own computer, comparing the current bytes on both sides so it only flags real differences, and offers to update each one individually — nothing is overwritten without your say-so. If any of the three required safety skills are missing from your workspace, it adds them back. This is a regular skill, not a protected one, so it can be updated the same way as any other skill, through the normal review-and-approve process. | builtin | 2026-07-22 | 1 | mandatory-infrastructure | cb33be6c1c2090eeaa9f90fa34b3ca8633eda62f5279c12cafd248aae746e2a2 |
...
| prompt-gate | Asks a few clarifying questions before answering, using your files for context. Improves how well it understands a vague prompt, and skips itself automatically for simple requests or when your message starts with `*`. | builtin | 2026-05-10 | 1 | study,research,writing,project-management,creative,business-admin,personal-assistant | 16b8ef1036d5d7320a7a166b5ea907d365a703b28f5858592bdccc810f1db2c3 |
...
| name | description | source_url | vetting_date | tier | goal_tags | sha256 |
|------|-------------|------------|--------------|------|-----------|----------------------------------------------------------------|
| anti-ai-slop | Breaks up text that reads like it was written by AI. Mixes up sentence lengths instead of the same-size rhythm AI tends to produce, cuts stock phrases, and removes hedging language you didn't ask for. Turned on only when you choose it, and it keeps your own writing voice instead of forcing everything into one fixed style. | builtin | 2026-07-19 | 1 | study,research,writing,project-management,creative,business-admin,personal-assistant | 92c56b918b6c52653d7e724f68fb1b52bbc63d8a4301a4c7d0a7c93ae682487d |
