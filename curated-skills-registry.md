...
|------|-------------|------------|--------------|------|-----------|----------------------------------------------------------------|
| self-apply | One of three required safety skills. It records every change you approve, in order, using a consistent format so entries stay easy to track, and requires you to propose a change, check it, and be able to reverse it before it takes effect. This file can never be edited through the kit's own change-approval process — it sits on a fixed, protected list that process always skips. Starting in v2.19, files whose names start with `self-` are protected the same way, along with the `context/.kit-migrations/` folder. | builtin | 2026-07-22 | 1 | mandatory-infrastructure | 0c77ab20779c79288eb35f3e1059955b566b3460456034b85dc87959a955e9f4 |
| self-archive | One of three required safety skills. It checks whether a file is safe to move before suggesting you archive something old or replaced, confirms the destination makes sense, and never deletes anything — it only moves a file into your archive folder, so nothing is lost and you can always move it back yourself. This file can never be moved by this or any other skill — it's on a fixed, protected list that the move process always skips. Starting in v2.19, `self-upgrade` and the `context/.kit-migrations/` folder are covered by the same protected list. | builtin | 2026-07-22 | 1 | mandatory-infrastructure | 7b12d467dbc1fc73cdd058b0d2bacb06fff7b8f9b07614fa2e478a30edf6d7f9 |
