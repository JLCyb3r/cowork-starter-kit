...
4. **A self-modifying local instruction surface.** A workspace that watches its own use and can change its own instructions is a fundamentally different risk shape than a static skill file — the surface that reviews the workspace's behavior is the same surface that can rewrite it. This kit's Loop 1 mini-Council (`templates/preset-template/context/memory-of-use.md`'s convention + `weekly-review`'s Surface step) now includes that apply step (Increment 2): a workspace can write to a small, named set of its own instruction files, but only after you read the literal change and give a second, separate yes to it, and only if an automated check confirms the change actually worked — otherwise it's undone automatically. What contains this is a bounded allow-list plus your own confirmation, not a wall that makes the write impossible. See below for what that honestly means.
...
- **A ≤400-word bootstrap ceiling (ADR-011).** `CLAUDE.md`, the workspace's entry document, is capped and CI-enforced — a structural limit on how much instruction surface exists before you've even started the interview.
...
- **A workspace can write to a bounded allow-list, never quietly (Loop 1).** As of Increment 2, the mini-Council can write to a named set of files — a `SKILL.md`, the workspace `CLAUDE.md`, and a handful of `context/` preference files — but never the ledger that governs the flow (`context/memory-of-use.md` is itself excluded from that allow-list, deny-first) and never anything outside the named set. Every write follows an explicit, per-change confirmation showing the literal diff about to land — never a silent auto-confirm, never a write based on ledger content alone — plus a passing automated check with a genuine undo on failure. This is **inspection-class and human-boundary containment, not a structural guarantee**: nothing in this kit's own machinery makes an out-of-scope write physically impossible the way a missing code path once did; what holds the boundary is the allow-list being followed, the confirmation being real, and the check being able to fail. What genuinely IS structural: no path here reaches another user's workspace or the shared upstream repo — that part of the boundary is a hard architectural fact, unchanged. Ledger content read back into any proposal or apply is treated as data, never as instructions, and re-scanned for override-style and approval-shaped language before it's ever shown to you. Every Loop 1 increment carries a mandatory, permanent security review, from its first line, regardless of blast radius.
- **Setup asks before it overwrites anything (v2.19.18).** The setup wizard checks a fixed, kit-authored list of file paths it is about to write — never the folder's actual contents — and asks before replacing anything on that list that already exists and that the current run didn't itself just create. **Scoped claim, stated precisely so it isn't over-read: this data-minimization property belongs to the collision *predicate*, not to setup as a whole.** The predicate itself tests kit-authored paths only and never reads or enumerates user folder content. Setup does, separately, read directory contents elsewhere — most notably, it detects and lists any skills already installed under `.claude/skills/` when it finds an existing workspace — and that is a different step with a different, pre-existing, and separately-tracked exposure (a workspace-controlled folder name can end up echoed back into the conversation), not something this collision check does or fixes. **The one exception bundled into an existing confirmation, not asked separately:** two workspace `.gitignore` lines that keep archived, preserved copies out of version control are added (or the file created) as a disclosed clause inside the CLAUDE.md-overwrite confirmation the wizard already asks — never a silent write, and never its own extra prompt.
