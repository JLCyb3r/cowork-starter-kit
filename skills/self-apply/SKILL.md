...
Before any entry's `Note` text is quoted into a proposal, it is re-scanned with the forbidden-imperative-token recipe this repo already uses at `CONTRIBUTING.md § Worked-example authoring rules, rule 2`. Any match is rendered inline, flagged, and never treated as a reason to skip or alter the confirmation flow — a note that reads like an instruction is still just a note.
