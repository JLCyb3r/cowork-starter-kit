...
3. **Malformed-registry-row refusal (AC-S5a, symmetric with the manifest clause above).** Before treating any of the three safety-skill rows (`self-apply`, `self-archive`, `self-upgrade`) as a valid verification source, confirm the row for that slug in `curated-skills-registry.md` is well-formed — exactly 9 pipe-delimited fields, with a 64-character lowercase-hex value in field 8. If it is not — a pipe-shifted row, a missing field, an uppercase or short hex value, or any shape that leaves "that slug's `sha256` entry" undefined — **REFUSE to offer or apply the affected backfill** and render the plain-language safe-fallback: "this skill's registry entry can't be verified safely — the row is damaged, so installing from it is refused." Never treat a malformed field 8 as verified, never compare against the wrong field, and never silently skip verification and proceed anyway. **Undefined is not fail-closed** — this rule exists precisely because "undefined" was previously a reachable state (`docs/risk-register.md` `v2.19.11-PULL-ROW-1`).
...
- **Treating a damaged registry row's field 8 as a verifiable hash.** A row that is not exactly 9 pipe-delimited fields with a 64-char lowercase-hex value in field 8 is refused (AC-S5a), never compared against, never coerced into "close enough."
