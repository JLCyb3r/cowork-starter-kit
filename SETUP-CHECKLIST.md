...
This is the paste-based setup route. The other route is: open the `cowork-starter-kit` folder as a Cowork Project and ask Cowork to read `CLAUDE.md` if the setup wizard does not begin on its own.
...
**The wizard** installs skills from the unified pool automatically during onboarding, writing them into your workspace's `.claude/skills/` — this happens regardless of upload, because the wizard writes the files directly. Project-folder `.claude/skills/` auto-discovery by Cowork itself (without the wizard's write) is a cloud-session behavior, not something a local Cowork session does; `skills-as-prompts.md` exists as a fallback for surfaces without discovery. If you skipped the wizard or work outside a connected folder and want to add skills manually:
