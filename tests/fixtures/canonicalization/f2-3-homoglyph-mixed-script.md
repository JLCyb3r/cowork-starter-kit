<!-- Fixture: F2-3 — Cyrillic homoglyph substitution, mixed-script FLAG (MF-S-2, AC-F2-3).
     ...
     Check-That-Cannot-Fail pattern). Verify with two legs:
       RED leg  — run the ADR-055 scan (CONTRIBUTING.md § Worked-example authoring rules, rule 2 recipe) against
     ...
     CONTRIBUTING.md § Worked-example authoring rules, rule 2 and scripts/canonicalize-scan.sh for the actual
     recipe.
