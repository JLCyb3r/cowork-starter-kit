...
**Resume guard — see WIZARD.md's Collision Rule (fail-safe posture / its interrupted-run resume exemption):** if `cowork-profile.md` exists with `Status: in-progress`, do NOT reset — resume per WIZARD.md Fallback (recap goal + bundle, continue from the first unanswered field). This branch marks `cowork-profile.md` `created-this-run` in the ledger before anything else, per the Collision Rule's narrow resume exemption — reaching this guard with `Status: in-progress` is itself proof the wizard wrote the stub, so re-asking about it would livelock.

**Reset guard — see WIZARD.md's Collision Rule:** if `cowork-profile.md` exists with real completed content, first offer the friendly menu: "You already have a workspace set up. 1) Keep it as-is  2) Add or remove skills  3) Start fresh from a new goal". Only option 3 triggers the reset confirmation, and that confirmation names what it actually resets — the profile only, never a stand-in for the Collision Rule's own per-path guarded-set confirmations, which still fire independently once the new run reaches F4: "This will reset your `cowork-profile.md` and re-run onboarding from Q1. It does not, by itself, authorize replacing any other file already in this workspace — the setup interview will name and ask about those separately if it finds them. Your past sessions are unaffected. Confirm? (Yes / No)" — proceed only on Yes; on No, return to the menu.
