...
- [ ] **Publish the Release AT THE FEATURE-MERGE COMMIT, BEFORE the Phase-8 retro merge lands, and BEFORE pushing the tag:** run `bash scripts/publish-release.sh` on `main` at the commit you intend to tag — this repo's unanimous precedent tags at the feature-merge commit, never the later retro-merge commit (`v2.18.0`→`5620730`, `v2.19.0`→`16e84f8`, `v2.19.2`→`c6648af`, `v2.19.3`→`f995aeb`). `v2.19.4` and `v2.19.5` were each merged, retro'd, and never tagged at all — twice consecutively — precisely because this step has no enforcement of its own; see the standing gate below. `publish-release.sh` extracts the dated `## [x.y.z]` section from `CHANGELOG.md`, creates the tag and a populated GitHub Release together in one step, and asserts the body names its version both before AND after the asset-upload workflow runs — never push a version tag with plain `git push` first (`release-assets.yml`'s asset-upload job will refuse to run against a Release it didn't find already populated; see ADR-076).
- [ ] **The standing release-surface gate is green** (`.github/workflows/release-surface.yml`, `scripts/verify-release-surface.sh`, ADR-078) — it re-runs automatically on every push to `main` and daily via `schedule`, and can be re-run on demand via `workflow_dispatch` after a manual publish (Scope A style backfill) that produced no commit of its own. It checks every dated CHANGELOG version at or above `v2.18.0` for a tag on origin, a Release whose body names its version, and that `/releases/latest` resolves to the newest one — a RED result names which of `MISSING-TAG`, `MISSING-RELEASE`, `DEFECTIVE-RELEASE-BODY`, or `WRONG-LATEST` failed, each with its own remedy. **What this gate does NOT check: assets.** If `publish-release.sh` halts after its asset poll times out (see the delete-and-recreate remedy the script itself prints — S8), the tag and Release it already created are public and permanent, and this gate will report that version GREEN regardless, because the asset conjunct is outside its predicate entirely. `v2.12.0` is the live precedent: it sits at 0 assets today, after a failed run, and would read as clean release-surface state if checked right now. A green standing gate is not evidence a release has its assets — check `gh release view v<X> --json assets -q '.assets | length'` separately if that matters for the release you're publishing.
