...
- [ ] **Publish the Release AT THE FEATURE-MERGE COMMIT, BEFORE the Phase-8 retro merge lands, and BEFORE pushing the tag:** run `bash scripts/publish-release.sh` on `main` at the commit you intend to tag — this repo's unanimous precedent tags at the feature-merge commit, never the later retro-merge commit (`v2.18.0`→`5620730`, `v2.19.0`→`16e84f8`, `v2.19.2`→`c6648af`, `v2.19.3`→`f995aeb`). `v2.19.4` and `v2.19.5` were each merged, retro'd, and never tagged at all — twice consecutively — precisely because this step has no enforcement of its own; see the standing gate below. **v2.19.7 (ADR-079):** `publish-release.sh` extracts the dated `## [x.y.z]` section from `CHANGELOG.md`, builds both release archives from the exact commit being tagged, and creates the tag, the populated Release, AND attaches both archives — all in one step, on all three of the script's internal branches (create/repair/idempotent-skip). There is no longer a separate asset-upload workflow to wait on: `.github/workflows/release-assets.yml` is verification-only (it downloads the published assets and re-asserts their content after the fact) and holds no write permission at all, so it cannot create or repair a Release — never push a version tag with plain `git push` first; run `publish-release.sh` before the tag exists, exactly as before.
- [ ] **The standing release-surface gate is green** (`.github/workflows/release-surface.yml`, `scripts/verify-release-surface.sh`, ADR-078) — it re-runs automatically on every push to `main` and daily via `schedule`, and can be re-run on demand via `workflow_dispatch` after a manual publish (Scope A style backfill) that produced no commit of its own. It checks every dated CHANGELOG version at or above `v2.18.0` for a tag on origin, a Release whose body names its version, and that `/releases/latest` resolves to the newest one — a RED result names which of `MISSING-TAG`, `MISSING-RELEASE`, `DEFECTIVE-RELEASE-BODY`, or `WRONG-LATEST` failed, each with its own remedy. **What this gate does NOT check: assets.** `publish-release.sh` asserts its own asset count synchronously and fails loudly if it is ever wrong (ADR-079 — there is no poll to time out anymore, and the old delete-and-recreate remedy this note used to point to has been removed along with the poll it existed to recover from), but the standing release-surface gate itself still does not check assets — its predicate covers tag + Release-body only. `v2.12.0` is the historical precedent this gap was named from: it sits at 0 assets from a pre-v2.19.7 failed run, and would still read as clean release-surface state if checked right now. A green standing gate is not evidence a release has its assets — check `gh release view v<X> --json assets -q '.assets | length'` separately if that matters for the release you're publishing.
- [ ] `vendored-integrity-check` is green — vendored tree matches `cowork.lock.json`
- [ ] **Run the offline smoke test** (`tests/offline-smoke-test.md`) — all 4 scorecard rows filled with real timed-run data, and the "15 minutes" hero claim's disposition matches the pre-bound decision rule applied to that data. A release tagged with a stale or empty smoke-test scorecard is shipping the timing claim on zero evidence. This obligation fires once per tag pushed — publishing multiple versions in one sitting (e.g. backfilling missed releases) means checking it that many times, not once for the batch.
...
and `require_code_owner_reviews: false` (verified 2026-08-30). Since v2.19.15, real CI runs on these
PRs — the checks in `.github/workflows/quality.yml` — but `main`'s branch protection does not
require any check to pass before merge, so nothing in CI actually gates the merge; the manual review
checklist below, satisfied by the sole maintainer, is the only control here. CODEOWNERS (see
`.github/CODEOWNERS`) records ownership and routes review; it does not currently enforce approvals.
